...
class TrialProbUseCase {
  ...
  // Aplica a Regra de Escadinha em toda a sequência
  Map<String, dynamic> _applyLadder() {
    ...
    int noCount = 0;
    // Lógica para identificar escadinhas (sequência crescente ou decrescente)
    for (int i = _history.length - 1; i > 1; i--) {
      // Verifica se há uma sequência crescente (KEY1 → KEY2)
    ...
    // Verifica se o padrão de escadinha foi encontrado em mais de 50% do histórico
    ...
    if (frequencyPercentage.round() >= minProbability) {
      var lastThree = _history.sublist(0, 2); // Pega as 2 ultimas jogadas
  ...
  // Aplica a Regra de Bate e toma em toda a sequência
  Map<String, dynamic> _applyHitAndTake() {
    ...
    int noCount = 0;
    // Lógica para identificar o padrão Bate e Toma
    for (int i = _history.length - 1; i > 1; i--) {
    ...
    if (frequencyPercentage.round() >= minProbability) {
      var lastTwo = _history.sublist(0, 2); // Pega as 2 ultimas jogadas
  ...
  Map<String, dynamic> _applyTicTac() {
    ...
    int noCount = 0;
    // Lógica para identificar o padrão HitAndTake
